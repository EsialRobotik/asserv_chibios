 ...
 */
Md22::Md22(bool is1motorRight, bool invertMotorRight, bool invertMotorLeft, I2cPinInit pins, int freq) :
        MotorController()
{
    m_i2cPinConf = pins;
    ...
    }else if (m_freq > 100 and m_freq <= 400) {
...
void Md22::init()
{
    ...
    cmd[0] = MD22_ACCREG;
    cmd[1] = 0;
    msg = i2cMasterTransmitTimeout(&I2CD1, MD22_ADDRESS, cmd, sizeof(cmd), NULL, 0, tmo);
    chDbgAssert(msg == MSG_OK, "Config MD22 - i2cMasterTransmitTimeout ERROR NOK\r\n");

    // Set motor speed to zero
    cmd[0] = MD22_MOTOR1REG;
    cmd[1] = 0;
    msg = i2cMasterTransmitTimeout(&I2CD1, MD22_ADDRESS, cmd, sizeof(cmd), NULL, 0, tmo);
    chDbgAssert(msg == MSG_OK, "Config MD22 - i2cMasterTransmitTimeout ERROR NOK\r\n");

    cmd[0] = MD22_MOTOR2REG;
    cmd[1] = 0;
    msg = i2cMasterTransmitTimeout(&I2CD1, MD22_ADDRESS, cmd, sizeof(cmd), NULL, 0, tmo);
    chDbgAssert(msg == MSG_OK, "Config MD22 - i2cMasterTransmitTimeout ERROR NOK\r\n");
...
void Md22::setMotorLeftSpeed(float percentage)
{
    ...
    i2cAcquireBus (&I2CD1);
    i2cMasterTransmitTimeout(&I2CD1, MD22_ADDRESS, cmd, sizeof(cmd), NULL, 0, TIME_MS2I(1));
    i2cReleaseBus(&I2CD1);
}
